...
import { checkBackendConnection, startConnectionRetry } from './utils/backendChecker'
...
const setupApiMock = () => {
  ...
  window.fetch = async function(resource, options) {
    ...
    if (typeof resource === 'string' && resource.startsWith('/api/')) {
      // 检查我们是否应该使用模拟数据
      const useMock = window._useMockData || window.localStorage.getItem('useMockData') === 'true'
      ...
      try {
        ...
        console.warn(`API请求失败 (${resource}): ${error.message}，切换到模拟数据`)
  ...
  return new Promise(resolve => {
    ...
    setTimeout(() => {
      ...
      if (url.includes('/api/status')) {
        ...
        responseData = {
          logs: [
            {time: '2023-09-15 12:00:00', level: 'INFO', message: '这是模拟日志数据', source: 'system'},
            {time: '2023-09-15 12:01:00', level: 'WARNING', message: '您正在使用模拟数据模式', source: 'system'},
            {time: '2023-09-15 12:02:00', level: 'INFO', message: '启动后端服务可获取真实数据', source: 'system'},
          ]
        }
      }
